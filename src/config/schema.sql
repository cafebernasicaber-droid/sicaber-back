...
CREATE TABLE IF NOT EXISTS insumos (
  ...
  -- automáticamente en POST /insumos según el local de trabajo del usuario y
  -- es inmutable. El nombre del insumo es único POR LOCAL (no global). FK
  -- agregada en config/db.js (la tabla "locales" se crea más abajo).
  local_id        INTEGER,
  created_at      TIMESTAMP DEFAULT NOW()
);
-- Nombre de insumo único POR LOCAL (no global): el mismo nombre puede
-- existir en dos locales distintos, cada uno con su propio stock. Parcial
-- (WHERE local_id IS NOT NULL) para no bloquear insumos sin local asignado.
...
CREATE TABLE IF NOT EXISTS pedidos (
  ...
  -- (estado 'en_camino'). atendido_por = cajero/bartender autenticado que
  -- hizo la transición (automático); domiciliario_id = repartidor asignado
  -- (opcional, solo domicilio). Ambos → usuarios.id. Reemplazan a las
  -- columnas de texto barista/domiciliario (eliminadas en config/db.js).
  atendido_por         INTEGER REFERENCES usuarios(id) ON DELETE SET NULL,
  domiciliario_id      INTEGER REFERENCES usuarios(id) ON DELETE SET NULL,
...
-- "Atendido por" / "Domiciliario" por id (reemplazan a las viejas columnas
-- de texto barista/domiciliario, que config/db.js elimina). Se agregan por
-- ALTER para bases creadas antes de que estuvieran en el CREATE TABLE.
ALTER TABLE pedidos ADD COLUMN IF NOT EXISTS atendido_por    INTEGER REFERENCES usuarios(id) ON DELETE SET NULL;
ALTER TABLE pedidos ADD COLUMN IF NOT EXISTS domiciliario_id INTEGER REFERENCES usuarios(id) ON DELETE SET NULL;
